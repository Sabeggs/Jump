...
import static utilz.Constants.Projectiles.*; // Assuming CANNON_BALL_DMG is here
...
public class ObjectManager {
    ...
    public void checkSpikesTouched(Player p) {
        for (Spike s : currentLevel.getSpikes())
            if (s.getHitbox().intersects(p.getHitbox()))
                // Change to use player.hit() for spike damage
                p.hit(); 
    }
    ...
    public void applyEffectToPlayer(Potion p) {
        // CHANGED: Red potion now calls addLife()
        if (p.getObjType() == RED_POTION)
            playing.getPlayer().addLife(RED_POTION_VALUE); // Add life
        else // This applies to BLUE_POTION
            playing.getPlayer().changePower(BLUE_POTION_VALUE); // Restore power
    }
        ...
        for (Projectile p : projectiles)
            if (p.isActive()) {
                ...
                if (p.getHitbox().intersects(player.getHitbox())) {
                    // CHANGED: Cannon ball now calls player.hit()
                    player.hit(); 
                    p.setActive(false);
